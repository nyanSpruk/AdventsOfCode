            ...
            for (int i = x_int; i < x_int + numberLen && !numFound; i++) {
                for (int x = Math.max(0, i - 1); x < Math.min(rows, i + 2) && !numFound; x++) {
                    for (int y = Math.max(0, j - 1); y < Math.min(cols, j + 2) && !numFound; y++) {
                        if (!(x == i && y == j) && !Character.isDigit(input.get(y).charAt(x)) && input.get(y).charAt(x) != '.') {
                            sum += numbers.get(numCoords);
                            numFound = true;
                        }
                    }
                }
    ...
    private static int oldSolve(List<String> input) {
        ...
        for (String numCoords : numbers.keySet()) {
            ...
            for (int i = x; i < x + numberLen; i++) {
                ...
                // Check up
                if (y - 1 >= 0 && (input.get(y - 1).charAt(i) != '.' && !(input.get(y - 1).charAt(i) >= '0' && input.get(y - 1).charAt(i) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }
                // Check down
                if (y + 1 < input.size() && (input.get(y + 1).charAt(i) != '.' && !(input.get(y + 1).charAt(i) >= '0' && input.get(y + 1).charAt(i) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }
                // Check left
                if (i - 1 >= 0 && (input.get(y).charAt(i - 1) != '.' && !(input.get(y).charAt(i - 1) >= '0' && input.get(y).charAt(i - 1) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }
                // Check right
                if (i + 1 < input.get(y).length() && (input.get(y).charAt(i + 1) != '.' && !(input.get(y).charAt(i + 1) >= '0' && input.get(y).charAt(i + 1) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }
                // Check diagonally up letf
                if (y - 1 >= 0 && i - 1 >= 0 && (input.get(y - 1).charAt(i - 1) != '.' && !(input.get(y - 1).charAt(i - 1) >= '0' && input.get(y - 1).charAt(i - 1) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }

                // Check diagonally up right
                if (y - 1 >= 0 && i + 1 < input.get(y).length() && (input.get(y - 1).charAt(i + 1) != '.' && !(input.get(y - 1).charAt(i + 1) >= '0' && input.get(y - 1).charAt(i + 1) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }

                // Check diagonally down letf
                if (y + 1 < input.size() && i - 1 >= 0 && (input.get(y + 1).charAt(i - 1) != '.' && !(input.get(y + 1).charAt(i - 1) >= '0' && input.get(y + 1).charAt(i - 1) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }

                // Check diagonally up right
                if (y + 1 < input.size() && i + 1 < input.get(y).length() && (input.get(y + 1).charAt(i + 1) != '.' && !(input.get(y + 1).charAt(i + 1) >= '0' && input.get(y + 1).charAt(i + 1) <= '9'))) {
                    sum += numbers.get(numCoords);
                    break;
                }
            }
